        ...
        if (currentIndex < sequence.length - 1) {
            const timeout = setTimeout(() => {
                ...
                setTimeout(() => {
                    ...
                    setFlash(false);
                }, 500); // Wait for white screen to swap
            }, 2500); // View time before evolve
    ...
    return (
        ...
        >
            ...
            <div className="absolute inset-0 overflow-hidden">
                <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[200vmax] h-[200vmax] bg-[conic-gradient(from_0deg,transparent_0deg,rgba(255,255,255,0.1)_20deg,transparent_40deg)] animate-[spin_4s_linear_infinite]"></div>
            </div>

            <h2 className="text-4xl md:text-6xl font-display font-black text-white mb-10 z-10 text-center animate-pulse">
                {isEvolving ? 'QUOI ?!' : 'ÉVOLUTION RÉUSSIE !'}
            </h2>
            ...
            <div className="relative w-80 h-80 md:w-96 md:h-96 z-10">
                <motion.img 
                    ...
                    className="w-full h-full object-contain filter drop-shadow-[0_0_30px_rgba(255,255,255,0.8)]"
                    animate={isEvolving ? {
                        scale: [1, 1.1, 1],
            ...
            {!isEvolving && (
                ...
                >
                    <p className="text-slate-300 font-mono mb-4">Votre Pokémon est devenu plus puissant !</p>
...
const PokemonDetailModal = ({ pokemon, user, inventory, onClose, onAction, onToggleTeam }: any) => {
    ...
    // Filter Usable Items - STRICTEMENT RESTREINT (Pas de combat items ici)
    const usableItems = inventory.filter((i: Item) => 
        ['HEAL', 'EVOLUTION', 'EVOLUTION_MAX'].includes(i.effect_type) && i.quantity > 0
    );
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-in fade-in zoom-in-95 duration-200">
            <div className="bg-slate-900 border border-cyan-500/50 rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-hidden flex flex-col shadow-2xl">
                ...
                <div className="p-4 border-b border-slate-700 bg-slate-950 flex justify-between items-center">
                    <div>
                        <h2 className="text-2xl font-display font-bold text-white uppercase">{pokemon.name}</h2>
                        <div className="flex gap-2 text-xs font-mono mt-1">
                            <span className="text-cyan-400">NIV {pokemon.level}</span>
                            <span className="text-slate-500">|</span>
                            <span className="text-slate-400">ID #{pokemon.tyradex_id}</span>
                        </div>
                    </div>
                ...
                <div className="flex-1 overflow-y-auto p-0 md:p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    {/* Column Left: Visuals & Main Stats */}
                    <div className="flex flex-col items-center p-6 bg-slate-950/50 md:rounded-xl">
                         <img 
                            src={pokemon.sprite_url} 
                            className="w-48 h-48 object-contain drop-shadow-[0_10px_20px_rgba(0,0,0,0.5)] mb-4" 
                         />
                         ...
                         <div className="w-full mb-4">
                             <div className="flex justify-between text-[10px] text-slate-400 mb-1 font-bold">
                                 ...
                                 <span>{pokemon.current_xp} / {pokemon.next_level_xp || 100}</span>
                             </div>
                             <div className="w-full h-3 bg-slate-800 rounded-full overflow-hidden border border-slate-700">
                                 <motion.div 
                                    ...
                                    transition={{ duration: 0.8, ease: "easeOut" }} // Rapide mais progressif
                                 >
                         ...
                         {/* Action Rapide: Feed */}
                         <button 
                            onClick={() => onAction('feed', pokemon.id)}
                            disabled={(user?.global_xp || 0) < 100}
                            className="w-full bg-gradient-to-r from-blue-600 to-cyan-600 hover:from-blue-500 hover:to-cyan-500 text-white py-3 rounded-lg font-bold shadow-lg disabled:opacity-50 disabled:grayscale transition-all flex items-center justify-center gap-2 group active:scale-[0.98]"
                         >
                            <img src={`${ASSETS_BASE_URL}/xp.webp`} className="w-5 h-5 group-hover:scale-110 transition-transform"/>
                            DONNER 100 XP
                         </button>
                    </div>

                    {/* Column Right: Stats & Items */}
                    <div className="p-4">
                            ...
                            <div className="space-y-4">
                                    ...
                                    return (
                                        <div key={stat}>
                                            <div className="flex justify-between text-xs font-bold text-slate-300 mb-1">
                                                ...
                                                <span>{val} {stat === 'HP' ? `/ ${max}` : ''}</span>
                                            </div>
                                            <div className="h-2 bg-slate-800 rounded-full overflow-hidden">
                                                <motion.div 
                                                    className={`h-full ${stat==='HP'?'bg-green-500':'bg-yellow-500'}`} 
                                                    initial={{ width: 0 }}
                                ...
                                <div className="mt-6 pt-6 border-t border-slate-800">
                                    <button 
                                        ...
                                        className={`w-full py-2 rounded border font-bold text-xs uppercase tracking-wider ${pokemon.is_team ? 'border-red-500 text-red-400 hover:bg-red-900/20' : 'border-green-500 text-green-400 hover:bg-green-900/20'}`}
                                    >
                            ...
                            <div className="space-y-2 max-h-[300px] overflow-y-auto">
                                ...
                                {usableItems.map((item: Item) => (
                                    <div key={item.id} className="flex justify-between items-center bg-slate-800 p-2 rounded border border-slate-700">
                                        ...
                                        <button 
                                            onClick={() => onAction('use_item', pokemon.id, item.id)}
                                            className="bg-slate-700 hover:bg-cyan-600 text-white text-[10px] font-bold px-3 py-1.5 rounded active:scale-95 transition-transform"
                                        >
                                            UTILISER
                                        </button>
                                    </div>
...
const PokemonCard = ({ pokemon, onClick }: { pokemon: Pokemon, onClick: () => void }) => (
    <motion.div 
        ...
        onClick={onClick}
        className={`
            relative p-4 rounded-2xl cursor-pointer group transition-all hover:-translate-y-1
            ${pokemon.is_team ? 'bg-gradient-to-b from-slate-800 to-slate-900 border-2 border-cyan-500 shadow-[0_0_15px_rgba(6,182,212,0.3)]' : 'bg-slate-900 border border-slate-700 hover:border-slate-500'}
        `}
    >
        {pokemon.is_team && (
             <div className="absolute top-2 right-2 w-3 h-3 bg-green-500 rounded-full shadow-[0_0_10px_rgba(34,197,94,0.8)] animate-pulse"></div>
        )}
        
        <div className="flex justify-between items-start mb-2 opacity-50 text-[10px] font-mono">
            <span>NV {pokemon.level}</span>
            <span>#{pokemon.tyradex_id}</span>
        </div>

        <div className="flex justify-center my-2">
            <img src={pokemon.sprite_url} className="w-24 h-24 object-contain group-hover:scale-110 transition-transform drop-shadow-lg" />
        </div>

        <div className="text-center">
            <h3 className={`font-display font-bold text-sm truncate ${pokemon.is_team ? 'text-cyan-400' : 'text-white'}`}>{pokemon.name}</h3>
...
export const Collection: React.FC = () => {
  ...
  // Local name map for correcting "Pokemon #ID"
  ...
  useEffect(() => {
    ...
    // Fetch Gen 1 names once to correct any "Pokemon #9" issues
        ...
        try {
            ...
            if(Array.isArray(res.data)) {
                ...
                res.data.forEach((p: any) => {
                    map[p.pokedexId] = p.name.fr;
                });
                setNameMap(map);
            }
        } catch (e) {
            console.warn("Could not fetch Tyradex names list");
        }
    };
    fetchNames();
  }, []);

  // Compute corrected collection
  ...
  const handleAction = async (action: string, pokeId: string, itemId?: string) => {
      setLoading(true);
      try {
          const res = await axios.post(`${API_BASE_URL}/collection.php`, {
              action,
              user_id: user?.id,
              pokemon_id: pokeId,
              item_id: itemId
          ...
          if (res.data.success) {
              ...
              if(action === 'feed') {
                   useGameStore.setState(s => ({ user: s.user ? { ...s.user, global_xp: s.user.global_xp - 100 } : null }));
              }
          } else {
  ...
  return (
    <div className="w-full max-w-6xl mx-auto pb-20">
      ...
      <AnimatePresence>
         {evolutionSeq && (
             <EvolutionOverlay sequence={evolutionSeq} onClose={() => setEvolutionSeq(null)} />
         )}
      </AnimatePresence>
      ...
      <AnimatePresence>
        {selectedPokemon && (
            <PokemonDetailModal 
                ...
                onToggleTeam={(id: string) => handleAction('toggle_team', id)}
            />
        )}
      </AnimatePresence>
      ...
      <div className="flex flex-col md:flex-row justify-between items-center mb-8 bg-slate-900/80 p-6 rounded-2xl border border-slate-700 shadow-xl backdrop-blur-sm">
        <div className="flex items-center gap-4">
             <div className="w-16 h-16 bg-gradient-to-br from-cyan-500 to-blue-600 rounded-full flex items-center justify-center border-4 border-slate-800 shadow-lg">
                <img src={`${ASSETS_BASE_URL}/pokeball.webp`} className="w-10 h-10 animate-pulse-fast" />
             </div>
             <div>
                <h2 className="text-3xl font-display font-bold text-white tracking-wide">CENTRE POKÉMON</h2>
                <p className="text-slate-400 font-mono text-sm">Gérez votre équipe et vos évolutions</p>
             </div>
        </div>
        
        <div className="mt-4 md:mt-0 bg-slate-950 px-6 py-3 rounded-xl border border-cyan-500/30 flex items-center gap-4">
            <div className="text-right">
                <div className="text-[10px] text-slate-500 font-bold uppercase tracking-wider">RÉSERVE XP</div>
                <div className="text-2xl font-mono text-cyan-400 font-bold drop-shadow-[0_0_10px_rgba(34,211,238,0.5)]">
                    {user?.global_xp} XP
                </div>
            </div>
            <img src={`${ASSETS_BASE_URL}/xp.webp`} className="w-8 h-8" />
        </div>
      </div>

      <div className="mb-12">
          <h3 className="text-xl font-display font-bold text-cyan-400 mb-4 flex items-center gap-2">
              <span className="w-2 h-8 bg-cyan-500 rounded-full"></span>
              ÉQUIPE ACTIVE ({activeTeam.length}/3)
          </h3>
          ...
          {activeTeam.length === 0 ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  ...
                  {[...Array(3 - activeTeam.length)].map((_, i) => (
                      <div key={i} className="bg-slate-900/30 border border-slate-800 rounded-2xl flex items-center justify-center opacity-30">
      ...
      <div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-4">
              ...
              <div className="border-2 border-dashed border-slate-800 bg-slate-900/20 rounded-2xl flex flex-col items-center justify-center p-4 text-slate-600 hover:border-cyan-500/30 hover:text-cyan-500 cursor-pointer transition-colors min-h-[180px]">
